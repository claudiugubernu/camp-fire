...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex flex-col h-full bg-surface-950">
      {/* Header */}
      <header className="safe-top flex items-center justify-between px-5 pt-3 pb-3 border-b border-surface-800">
          ...
          {NAV_ITEMS.map((item) => (
            ...
            >
              {({ isActive }) => (
                ...
                >
                  <span className={`text-2xl transition-all ${isActive ? "scale-110" : "opacity-60"}`}>
          ...
          {state.user?.isAdmin && (
            <NavLink to="/admin" className="flex-1">
              {({ isActive }) => (
                ...
                >
                  <span className={`text-2xl ${isActive ? "" : "opacity-60"}`}>⚙️</span>
                  <span className={`text-[10px] font-semibold ${isActive ? "text-fire-400" : "text-text-muted"}`}>
                    Admin
                  </span>
                </motion.div>
              )}
